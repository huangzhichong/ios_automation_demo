...
class ElementBase
  ...
  def get_property(string_property)
    ...
    result.nil? ? result : result.strip
      ...
      Common.logger_error "Execute - get #{string_property} of #{self.class} - failed. can't find element by #{@@type} and #{@@value}"
    end
  end
  ...
  def wait_element_disappear(timeout=30)
    ...
    Common.logger_step "Execute - wait #{self.class} to disapper in #{timeout} secs - success."
  ...
  def wait_element_check_property(string_property,expected_result,timeout=30)
    ...
    Common.logger_step "Execute - wait #{string_property} of #{self.class} to change - success. the [#{string_property}] changed to [#{expected_result}]"
  end
