...
class RSpec::Core::Example
  ...
  def result!
    if self.failed?
      ...
      exception_backtrace = RSpec::Core::BacktraceFormatter.format_backtrace(@exception.backtrace, @metadata)
    ...
    @metadata[:description].split('_').first.split('##').each do |test_case_id|
      MARQUEE::update_case_result($round_id,$plan_name,test_case_id,$result,$errormessage,$screenshot,$errorlog)
    end
    reset_status
    temp.should == ""
  end
